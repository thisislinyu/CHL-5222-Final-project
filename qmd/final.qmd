...
missing_outcome_dat <- vaccine_dat %>%
  ...
  summarise(n_schools = n(),## total number observations in each states
    spend_person = mean(spend_person),
     miss_outcome_n =  sum(is.na(overall_recode)) , ## total number of schools with outcome missing
    miss_outcome_prop = mean(is.na(overall_recode)) * 100, ## percent of outcome missing in each state
            ) %>% 
  arrange(desc(miss_outcome_prop))
...
missing_mmr_dat <- vaccine_dat %>%
  ...
  summarise(n_schools = n(),## total number observations in each states
    spend_person = mean(spend_person),
     miss_outcome_n =  sum(is.na(mmr_recode)) , ## total number of schools with outcome missing
    miss_outcome_prop = mean(is.na(mmr_recode)) * 100, ## percent of outcome missing in each state
            ) %>% 
  arrange(desc(miss_outcome_prop))
