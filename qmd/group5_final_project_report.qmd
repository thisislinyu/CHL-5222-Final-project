...
```{r}
##** table 1 stratified by vaccination group **
...
##** summarize data completion for the different models **
##*
...
state_count_mmr <- vaccine_dat %>% filter(!is.na(mmr_recode)) %>% summarise(n_states = n_distinct(state))
state_count_overall <- vaccine_dat %>% filter(!is.na(overall_recode)) %>% summarise(n_states = n_distinct(state))
...
spending_mmr <- vaccine_dat %>%
  ...
  summarise(mean_spend_million = round(mean(spend, na.rm = TRUE) / 1000, 2))
...
table(vaccine_dat$year_recode[!is.na(vaccine_dat$mmr_recode)])      # For MMR
table(vaccine_dat$year_recode[!is.na(vaccine_dat$overall_recode)])  # For Overall
...
mean_overall <- vaccine_dat %>%
  filter(!is.na(overall_recode)) %>%
  summarise(mean_overall = round(mean(overall_recode, na.rm = TRUE), 1))
...
summary_table <- tibble(
  ...
  `County Count (Sum, Median, Min, Max)` = c(
  paste(county_stats_mmr$total, county_stats_mmr$median, county_stats_mmr$min, county_stats_mmr$max, sep = ", "),
  paste(county_stats_overall$total, county_stats_overall$median, county_stats_overall$min, county_stats_overall$max, sep = ", ")
),

`School Count (Sum, Median, Min, Max)` = c(
  paste(school_stats_mmr$total, school_stats_mmr$median, school_stats_mmr$min, school_stats_mmr$max, sep = ", "),
  paste(school_stats_overall$total, school_stats_overall$median, school_stats_overall$min, school_stats_overall$max, sep = ", ")
),
  `Spending per Person (Million USD)` = c(
    spending_mmr$mean_spend_million,
    spending_overall$mean_spend_million
  ),
